...
namespace POS.Shared.Caching
{
    public class DefaultCacheProvider : ICacheProvider
    {
        ...
        private const string Prefix = "VANCT_POS";
        ...
        {
            return Cache[string.Format("{0}_{1}", Prefix, key)];
        }
        ...
        public void Set(string key, object data, int cacheTime)
        {
            lock (key)
            {
                ...
                Cache.Add(new CacheItem(string.Format("{0}_{1}", Prefix, key), data), policy);
            }
        }
        ...
        {
            return (Cache[string.Format("{0}_{1}", Prefix, key)] != null);
        }
        ...
        public void Remove(string key)
        {
            lock (key)
            {
                if (IsSet(key)) Cache.Remove(string.Format("{0}_{1}", Prefix, key));
            }
        }
